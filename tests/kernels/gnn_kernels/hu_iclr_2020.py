...
from gprotorch.kernels.gnn_kernels.test_hu_iclr_2020 import mol_to_pyg, GNN
...
benchmark_mols = [mol_to_pyg(MolFromSmiles(smiles)) for smiles in benchmark_loader.features]
...
def test_embedding_generation(gnn_type, pretrain_method):
    ...
    node_embeds = torch.tensor_split(node_embeds, benchmark_batch.ptr[1:-1], dim=0)
